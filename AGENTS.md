...
4. **Service-role READS must filter by `user_id` too — not just writes.** This is the rule that has been violated most. Any helper that builds context, dedups, or gathers "existing items" by a *shared* key (`external_id`, `company_id`, `source_thread_id`, thread/meeting ids) on a service-role client **must also** `.eq("user_id", userId)`. Shared keys collide across tenants, so a query keyed only on them returns *other users' rows* — and if that result feeds an LLM (triage "existing items", enrichment context) it leaks one tenant's data into another's model context or drives a wrong mutation. Querying `s2d_items`/`meetings`/`messages` by `external_id` or `company_id` alone under service-role is a cross-tenant bug, full stop. (This is the AGENT_IMPROVEMENT / MULTITENANCY_AUDIT.md Fireflies-sync class of defect.)
5. **Service-role helpers that take an opaque `id` must self-scope or document the obligation.** A function like `getActiveAccessToken(connectionId)` that decrypts secrets / returns a row by id on a service-role client must either accept `userId` and `.eq("user_id", userId)`, or carry a loud comment that **every caller must verify ownership first**. An unscoped id-lookup is one careless caller away from account-takeover-grade IDOR.
6. **API routes derive identity from the session/token, never from client input.** Resolve `userId` via `supabase.auth.getUser()` (or `verifyToken` for `mashi_pat_` bearer) and return 401 if absent — *before* any DB call or LLM call. Never read `user_id` from the request body, query string, or URL param. Routes with an `[id]`/`[connectionId]` param must scope the row to the authenticated user (explicit `.eq("user_id", userId)` on the service-role client, or a session-client read that RLS scopes — plus an explicit ownership check, don't lean on RLS silently). LLM-invoking routes must gate auth first so spend is attributable.
7. **Browser-direct reads rely SOLELY on RLS — so RLS is not optional polish, it is the control.** Client components/hooks (`createSupabaseBrowserClient`, the `use-*` hooks) query tables directly with no `user_id` filter in the client code. Any new table reachable from the browser **must** ship with owner-only RLS (`auth.uid() = user_id` on `USING` **and** `WITH CHECK`, all four verbs) in the same migration that creates it. Never reach a browser-read table with the service-role client. Never write a policy with `USING (true)` / `WITH CHECK (true)` on a tenant table (see `ai_usage_log`'s INSERT for the one legacy exception we're paying down).

**Enforcement, not just documentation.** These rules were authored alongside features that do not yet fully comply — treat existing code as *not* grandfathered. `MULTITENANCY_AUDIT.md` is the live backlog of violations (T1–T18). Before shipping a feature that touches the DB: (a) run `supabase/tests/rls_assertions.sql` against your DB to confirm every tenant table is owner-only; (b) grep your diff for service-role queries lacking a nearby `user_id` filter; (c) if you add a new table/route/browser-read, add it to the audit's matrix. New cross-tenant readers without a `user_id` filter are the #1 way this app regresses.
