...
export async function collectWritingSamples(opts: {
  ...
  const collected: Array<{ provider: string; account_label: string; text: string }> = [];
  ...
  for (const c of connections ?? []) {
    try {
      let samples: string[] = [];
      if (c.provider === "gmail") {
        samples = await collectGmailSent(c.id, 15);
      } else if (c.provider === "slack") {
        samples = await collectSlackOwnMessages(c.id, c.account_email ?? "", 15);
      }
  ...
  // Interleave across providers/accounts so the final sample isn't lopsided
  // toward one inbox.
  const interleaved = interleave(collected.map((c) => c.text));

  // Deduplicate roughly-identical strings (auto-reply boilerplate slips through)
  ...
  return {
    samples: unique,
    perSource: {
      gmail: collected.filter((c) => c.provider === "gmail").length,
      slack: collected.filter((c) => c.provider === "slack").length,
    },
    perAccount,
  };
}
...
function interleave<T>(items: T[]): T[] {
  // Simple round-robin: items are already in connection-order. We just want
  // them mixed so trimming to N doesn't drop one inbox entirely.
  return items;
}
...
async function collectGmailSent(connectionId: string, target: number): Promise<string[]> {
  const token = await getActiveAccessToken(connectionId);
  const url = new URL(`${GMAIL_API}/users/me/messages`);
  url.searchParams.set("q", "in:sent newer_than:30d -from:noreply -from:no-reply");
  ...
  const bodies = await Promise.all(
    ids.map(async (id) => {
      try {
        ...
        return composeGmailSample(subject, text);
      } catch {
        return null;
      }
    })
  ...
  const filtered = bodies
    ...
    .map((s) => trimQuotedReplyTail(s))
    .filter(isLikelyHumanProse);
...
async function collectSlackOwnMessages(
  ...
  const convRes = await fetch(`${SLACK_API}/conversations.list?types=im&limit=100`, {
    headers: { Authorization: `Bearer ${token}` },
  });
  ...
  const oldestTs = ((Date.now() - 30 * 86_400_000) / 1000).toString();
